...
KJUR.crypto.EdDSA = function(params) {
...
var curveName = "id-Ed25519"; // curve name default
var prvKeyHex = null;
var pubKeyHex = null;
...
this.generateKeyPairHex = function() {
    const hPrv = KJUR.crypto.Util.getRandomHexOfNbytes(32);
    this.setPrivateKeyHex(hPrv);
    const hPub = this.generatePublicKeyHex();
    this.isPublic = false;
    return {'prvhex': hPrv, 'pubhex': hPub};
};
...
this.signWithMessageHash = function(msgHex) {
    return this.signHex(msgHex);
};
...
this.verifyWithMessageHash = function(msgHex, sigHex) {
    return this.verifyHex(msgHex, sigHex);
};
...
this.readPKCS8PrvKeyHex = function(h) {
    ...
    let obj = ASN1HEX.parse(h);
    if (obj.seq[1].seq[0].oid !== "id-Ed25519")
        throw new _Error("cannot find id-Ed25519");
    this.setNamedCurve(obj.seq[1].seq[0].oid);

    this.setPrivateKeyHex(obj.seq[2].octstr.obj.octstr.hex);
}
...
this.readPKCS8PubKeyHex = function(h) {
    ...
    let obj = ASN1HEX.parse(h);
    if (obj.seq[0].seq[0].oid !== "id-Ed25519")
        throw new _Error("cannot find id-Ed25519");
    this.setNamedCurve(obj.seq[0].seq[0].oid);

    let ec_point = obj.seq[1].bitstr.hex || obj.seq[1].bitstr.obj.asn1.tlv;
    if (ec_point.length == 66 && ec_point[0] === '0' && ec_point[1] === '0')
        ec_point = ec_point.substring(2);
    this.setPublicKeyHex(ec_point);
}
...
this.readCertPubKeyHex = function(h, nthPKI) {
    ...
    try {
        ...
        throw new _Error("malformed X.509 certificate EdDSA public key");
    }
...
function _getName(curve) {
    if (s === "2b6570") return "id-Ed25519"; // OID: 1.3.101.112
    if ("|ed25519|25519|".indexOf(s) !== -1) return "id-Ed25519";
};
